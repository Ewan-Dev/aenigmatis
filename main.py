...
while True:
    ...
    if command == "help":
    ...
    elif command == "vig_bruteforce": 
        print("Paste you ciphertext here: ")
        ciphertext = read_input()
        if os.path.exists("vignere_list.txt"):
            os.remove("vignere_list.txt")         
        txt = """the
        of
        and
        to
        ...
        for word in text_split:
            plaintext = vignere_decipher(ciphertext, word)
            score = overall_english_score(plaintext)
            all_texts.append((score, plaintext, word))
            print(word)
        sorted_data = sorted(all_texts, key=lambda x: x[0], reverse=True)
        f = open("vignere_list.txt", "w")
        f.write(str(sorted_data))
        f.close()
        print("Saved all possibilities in vignere_list.txt, ordered by English likelihood")
    elif command == "exit":
        exit()
    else:
